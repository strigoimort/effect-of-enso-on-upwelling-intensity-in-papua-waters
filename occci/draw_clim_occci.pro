...
;xsize=800 ;ukuran kanvas dalam pixel
;ysize=550
...
;path_input='D:\SKRIPSI\database\occci\neutral\compile climatology\chl2016'+month_st+'.sav'
;path_input='D:\SKRIPSI\database\occci\neutral\compile climatology\chl2016'+month_st+'.sav'
...
;compile clim yg bener dari ekstrak
;compile clim 2 yg kebalik di draw

;data_map1 = chl_map[a:b,c:d];data_map=[1225,721]
;data_map2 = rotate(data_map1,7)
;data_map = data_map2

data_map1=rotate(data_map,7);udh bener ni jgn diganti
;data_map2=shift(data_map1,50)
;data_map2=shift(data_map,-50)

; Get the size of the image returned by the SIZE function.
imageSize = SIZE(data_map1, /DIMENSIONS)

; Display the image using the IMAGE function.
;im = IMAGE(data_map1, TITLE = 'Original Image')

; Use the SHIFT function to correct the original image.
cell2 = SHIFT(data_map1, -(imageSize[0]/1), (imageSize[1]/3))

; Display the corrected image in a second window.
;im2 = IMAGE(cell2, TITLE = 'Shifted Image')
;im=data_map1
;data=data_map
data=cell2
;data=shift(data_map1,50)
...
;---------------------------contour interval
level_interval=0.01 ;interval konturnya, makin kecil makin halus
...
;===========================COLORBAR===========================
;interval_measure=0.2 ;interval warna di colorbar
...
;XYOUTs,0.09,0.6,'Latitude',/normal,alignment=0.5,charsize=3.,charthick=2.5,font=1, orientation=90, color='000000'xl ;tulisan latitude
;unit_st='(!U0!NC)'
unit_st='(mg/m!u3!n)'
;XYOUTs,0.89,0.08,unit_st,/normal,alignment=0.5,charsize=2.,charthick=2.0,font=1;,color=255 ;tulisan derajat

;print, max(data) & print, min(data)

;stop
;============================= 'E:\SKRIPSI\pasifik\klorofil\common\'+year_st+'\chl_'+year_st+month_st+'.sav'
;file_mkdir, 'D:\SKRIPSI\database\colorbar\
file_mkdir, 'D:\SKRIPSI\database\occci\neutral\climatology baru\
;file_mkdir, 'D:\SKRIPSI\database\occci\neutral\draw climatology\
;path_output ='D:\SKRIPSI\database\colorbar\chl'+month_st+'.png'
path_output ='D:\SKRIPSI\database\occci\neutral\climatology baru\chl_'+month_st+'.png'
;path_output ='D:\SKRIPSI\database\occci\neutral\draw climatology\chl2016_'+month_st+'.png'
;stop
;==================================================
